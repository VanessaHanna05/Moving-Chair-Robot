...
//--------------- USER SETTINGS ---------------------------------------
const char* ssid = "Hanna2G";          // WIFI network SSID or AP SSID
const char* password = "451200963";
...
int port =8000;
const char* serverIP = "192.168.100.150";  
String data="";     // The three first numbers have to be the same with the router IP
...
void setup() {
  ...
  // password = "11100000"; 
     ...
     // You have to set the baud rate of the ESP8266 to 9600 before 
...
void loop() {
  ...
  espSerial.println("AT+CWJAP?");  // configure as client
  waitForResponse("OK",1000);
   Serial.println("Received Response: " );
  Serial.println(data);
  Serial.println("Response parsed");

  int RSSIIndicatorIndex = data.indexOf("-");
  if(RSSIIndicatorIndex!=-1){
   String RSSIValue = data.substring(RSSIIndicatorIndex, RSSIIndicatorIndex+3);
   int RSSI = RSSIValue.toInt();
   Wire.beginTransmission(9); // Replace '8' with the I2C address of the receiver Arduino on uno
   Wire.write(RSSI);
   Wire.endTransmission();
   delay(1000);
   Serial.println("RSSI Value: "+ RSSIValue);
  }
  else{
    Serial.println("RSSI was not received");
  }
delay(5000);
